...
class MonthlyFeeController extends Controller
{
    ...
    {
    	Session::put('page', 'month_fee');
    	$years = Year::orderBy('id','desc')->get();
    	$classes = StudentClass::all();
    	return view('backend.student.monthly_fee.view-monthly-fee')->with(compact('years','classes'));
    }
    public function getStudent(Request $request)
    {
    	...
    	foreach ($allstudent as $key => $v) {
    		...
    		$html[$key]['tdsource'] .='<a class="btn btn-sm btn-'.$color.'" title="Payslip" target="_blank" href="'.route("students.month.fee.payslip").'?class_id='.$v->class_id.'&student_id='.$v->student_id.'&month='.$request->month.'">Fee Slip</a>';
    	    $html[$key]['tdsource'] .='</td>';
